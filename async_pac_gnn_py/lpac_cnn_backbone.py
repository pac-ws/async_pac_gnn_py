...
class LPACCNNBackbone:
    ...
    def __init__(self, config: dict, params: Parameters):
        ...
        self.model = CNNBackBone(self.config).to(self.device)
        self.model.eval()

    def get_communication_maps_from_positions(
        self, relative_positions: torch.Tensor, map_size: int
    ) -> torch.Tensor:
    ...
    def get_maps(self, env, relative_positions: torch.Tensor) -> torch.Tensor:
        ...
        if self.use_comm_map:
            comm_maps = self.get_communication_maps_from_positions(
                relative_positions, self.cnn_map_size
            )
    ...
    def step(self, env, relative_positions: torch.Tensor):
